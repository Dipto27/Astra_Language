 ...
 * memory.c — Memory management and mark-sweep garbage collector
 */
...
void* reallocate(void* pointer, size_t oldSize, size_t newSize) {
    ...
    if (result == NULL) {
        fprintf(stderr, "\n\033[1;31mFatal Error:\033[0m "
                "Out of memory — allocation of %zu bytes failed.\n",
                newSize);
        exit(1);
    }
    return result;
...
/* ── GC: Blacken (trace references from) an object ───────── */
